...
public class Morse extends AppCompatActivity implements GestureDetector.OnGestureListener,GestureDetector.OnDoubleTapListener{
    ...
    public void onLongPress(MotionEvent event) {
        ...
        if (countTocuh<10) {
            ...
            countTocuh++;
            if(countTocuh==1){
                mText2.setText(Integer.toString(countTocuh)+" er signal");}
            else{mText2.setText(Integer.toString(countTocuh)+" eme signal");}
            fin();
        }
    }
    ...
    public void fin() {
        ...
        if (countTocuh==10 || (essai.compareTo(modele)==0)) {
            ...
            if (essai.compareTo(modele)==0) {
                mText3.setText("Vous avez gagné... chanceux");
                ...
                mText3.setText("Vous avez perdu... 30 sec en plus !");
                final Handler handler = new Handler();
                handler.postDelayed(new Runnable() {
                    @Override
                    public void run() {
    ...
    public void changement_fin()
    {
        ...
        if (score_time > 60) {
            com_score.setText("Toi, tu vas finir chez Capge...");
            playSound(R.raw.lose);
        }
        else if (score_time > 30) {
            com_score.setText("Et un énième maïs humain...");
            playSound(R.raw.lose);
        } else if(score_time<30 && score_time >15)
        {
            com_score.setText("Pas mal! tu ferais un bon spacionaute");
            playSound(R.raw.appl5);
        }else if(score_time<15)
        {
            com_score.setText("tu es un dieu de la transmission galactique");
